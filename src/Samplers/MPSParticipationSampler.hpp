  ...
  public:
    ...
    virtual void add_samples(dataframe::SampleMap& samples, const std::shared_ptr<QuantumState>& state_) override {
      ...
      if (sample_configurational_entropy || sample_configurational_entropy_mutual || sample_configurational_entropy_bipartite) {
        ...
        auto bit_samples = state->sample_bitstrings(supports, num_configurational_entropy_samples);
        ...
        if (sample_configurational_entropy_mutual) {
          ...
          dataframe::utils::emplace(samples, "configurational_entropy_mutual", L);
        }

        if (sample_configurational_entropy_bipartite) {
          std::vector<double> L = state->process_bipartite_bit_samples(bit_samples);
          dataframe::utils::emplace(samples, "configurational_entropy_bipartite", L);
        }
      }
    }
};
