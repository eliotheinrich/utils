...
class FreeMajoranaState : public EntropyState {
  private:
    size_t L;

  public:
    Eigen::MatrixXcd amplitudes;
    FreeMajoranaState(size_t L) : L(L), EntropyState(L) {
      particles_at({});
    }
...
class FreeFermionSimulator : public Simulator {
  ...
  public:
    std::shared_ptr<FreeMajoranaState> state;
    FreeFermionSimulator(dataframe::ExperimentParams& params, uint32_t num_threads) : Simulator(params), sampler(params) {
      ...
      state = std::make_shared<FreeMajoranaState>(L);
    }
